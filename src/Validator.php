...
use Forme\Platine\Exception\TemplateErrorException;
...
final class Validator
{
    ...
    public static function validateFile(string $path): void
    {
        ...
        array_map(function (array $rule) use ($contents, $path): void {
            $errorLine = self::checkAndReturnLine($rule['pattern'], $contents);
            if ($errorLine) {
                throw new TemplateErrorException(message: $rule['message'], filename: $path, line: $errorLine);
            }
        }, self::RULES);
    }
