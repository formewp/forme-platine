...
function componentFunc($insert = null)
{
    ...
    return startBufferFunc(function (FuncArgs $args) use ($insert): \Closure {
        ...
        return function ($contents) use ($insert, $args): void {
            [$name, $data] = $args->args;
...
function slotFunc()
{
    return startBufferFunc(function (FuncArgs $args): \Closure {
        if ($args->template()->get('component_slot_data') === null) {
            throw new FuncException('Cannot call slot func outside of component definition.');
        }

        return function ($contents) use ($args): void {
...
{
    return function (FuncArgs $args) use ($create_callback): void {
...
function endFunc()
{
    return function (FuncArgs $args): void {
        $buffer_stack = $args->template()->get('buffer_stack') ?: [];
...
function insertFunc($echo = null)
{
    ...
    return function (FuncArgs $args) use ($echo): void {
        [$name, $data]     = $args->args;
...
{
    return function (FuncArgs $args): array {
        [$data] = $args->args;
...
function wrapSimpleFunc(callable $func, $enable_bc = false)
{
    return function (FuncArgs $args) use ($func, $enable_bc) {
        if ($enable_bc && is_array($func) && isset($func[0]) && $func[0] instanceof ExtensionInterface) {
...
{
    return fn (FuncArgs $args): string => htmlspecialchars((string) $args->args[0], $flags, $encoding);
}
...
function splitByNameFunc(array $handlers)
{
    return function (FuncArgs $args, $next) use ($handlers) {
        ...
        if (isset($handlers[$name])) {
            $handler = stackGroup($handlers[$name]);
